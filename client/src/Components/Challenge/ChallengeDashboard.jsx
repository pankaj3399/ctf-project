...
const ChallengeDashboard = ({ activeCategory, activeSubCategory, promptLoading, datas }) => {
    ...
    useEffect(() => {
        if (activeCategory && challenges?.length) {
            setFilterChallenges(challenges.filter((item) => item.category.name === activeCategory?.name))
    ...
    return (
        <div className="relative w-full h-full pb-5 mt-5 overflow-y-auto pr-5">
            ...
            {promptLoading ? <div className="flex justify-center items-center h-1/2 tracking-wider text-[32px] font-semibold"> Loading...</div> : filteredChallenges.length === 0 ?
